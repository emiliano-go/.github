Hi, I'm Emiliano.

From Montevideo, Uruguay. I write Python for a living and spend most of my free time doing the same thing.

I'm currently focused on creating and maintaining **[DBWarden](https://github.com/dbwarden-org/DBWarden)**, a SQLAlchemy-native migration library. The idea is that your models already define your schema, so they should be the source of truth for migrations too. Still iterating on it.

I also lead **[LibreCourseUY](https://librecourse.uy)**, an open source organization trying to grow the OSS culture in Uruguay.

My stack is Python, FastAPI, SQLAlchemy, ClickHouse, Docker and PostgreSQL. I self-host everything on a VPS because I like understanding what's actually running. Arch Linux btw :D

I learn by building things, breaking them, and figuring out why. It's slow but it sticks.

---

*[blog](https://blog.emiliano-go.com) · [site](https://emiliano-go.com) · [librecourse.uy](https://librecourse.uy)*
